...
from .stage2_scene import compose_scene
...
def run(args: argparse.Namespace) -> str:
    ...
    if not args.skip_scene:
        print("[2/4] Compositing scene (local FLUX Kontext)...")
        compose_scene(str(cutout_path), args.scene_prompt, str(hero_path), quantize=args.quantize)
    else:
...
def main():
    ...
    parser.add_argument("--quantize", type=int, default=4, choices=[3, 4, 6, 8], help="FLUX quantization (lower = less RAM)")
